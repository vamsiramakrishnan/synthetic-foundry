...
def compile_dataset(
    ...
    batch_limit: int | None = None, replay_only: bool = False,
) -> DatasetRun:
    ...
    for index in range(budget):
        ...
        cell = min((s for s in plan.strata if remaining[s.id]), key=lambda s: (
            attempts[s.id], -Fraction(remaining[s.id], s.count), s.id))
        request = DatasetRequest(
            plan_digest=plan_key, stratum=cell.id, ordinal=attempts[cell.id],
            seed=int(digest([plan.seed, cell.id, attempts[cell.id]])[:8], 16),
