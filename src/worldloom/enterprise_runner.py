"""MCP-neutral execution adapter with resumable deterministic checkpoints."""
...
class MCPInvoker(Protocol):
    def __call__(self, tool_name: str, arguments: Mapping[str, Any]) -> Awaitable[Mapping[str, Any]]: ...
...
class RunnerConfig(Model):
    ...
    def resolve(self, connector: str, operation: str, entity: str) -> str:
        ...
        for binding in self.bindings:
            if (binding.connector, binding.operation, binding.entity) == (connector, operation, entity):
                return binding.tool_name
        raise KeyError(f"no MCP binding for {connector}.{operation}.{entity}")
...
def shard_queries(queries: tuple[PlannedEnterpriseQuery, ...], index: int, count: int) -> tuple[PlannedEnterpriseQuery, ...]:
    if count < 1 or not 0 <= index < count:
        raise ValueError("shard requires count >= 1 and 0 <= index < count")
    return tuple(query for position, query in enumerate(queries) if position % count == index)
    ...
    *,
    ...
    for node in query.expected_dag:
        ...
        if any(dependency not in outputs for dependency in dependencies):
            result = ExecutionResult(query_id=query.id, calls=tuple(calls), outputs=outputs, completed=False, finding=f"unresolved dependencies for {node['id']}")
            if checkpoint:
                checkpoint(result)
            return result
        tool_name = config.resolve(node["connector"], node["kind"], node["entity"])
        ...
        calls.append(TraceCall(id=content_key("trace-call", query.id, node["id"]), connector=node["connector"], operation=node["kind"], entity=node["entity"], depends_on=dependencies, record_id=response.get("record_id"), fact_ids=tuple(response.get("fact_ids", ())), succeeded=bool(response.get("succeeded", True))))
        partial = ExecutionResult(query_id=query.id, calls=tuple(calls), outputs=outputs, completed=False)
        if checkpoint:
            checkpoint(partial)
        if not calls[-1].succeeded:
            return partial.model_copy(update={"finding": f"node {node['id']} failed"})
    result = ExecutionResult(query_id=query.id, calls=tuple(calls), outputs=outputs, completed=True)
    if checkpoint:
        checkpoint(result)
    return result
...
def load_checkpoint(path: Path) -> ExecutionResult | None:
    if not path.exists():
        return None
    return ExecutionResult.model_validate(json.loads(path.read_text(encoding="utf-8")))
