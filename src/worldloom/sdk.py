...
__all__ = [
    "Blueprint", "Built", "banking", "cross", "dispersed", "engine", "insurance",
    "mosaic_of", "probe_of", "retail", "sweep",
]
...
def dispersed(
    ...
    *,
    ...
    vectors = [list(key(b)) if key else _coordinates(b) for b in candidates]
    if key is None:
        vectors = _normalised(vectors)
    chosen = farthest_first(vectors, manhattan, count)
    return tuple(candidates[at] for at in chosen)


def _coordinates(blueprint: Blueprint) -> list[float]:
    described = blueprint.describe()
    shape = described["shape"] or {}
    physics = described["physics"]
    return [
        float(shape.get("headcount", 0)), float(shape.get("span", 0)),
        float(shape.get("levels", 0)),
        float(profiles.MONTHS.index(1) if described["calendar"] is None
              else sorted(profiles.PROFILES).index(described["calendar"])),
        float(("none", "small", "medium", "large").index(described["estate"] or "none")),
        *(float(entry["low"]) for _, entry in sorted(physics.items())),
    ]
