...
from . import Rendered, chaptered, slug_for
...
def _anchor(heading: str) -> str:
    """The fragment a Markdown viewer gives a heading — lower case, punctuation
    dropped, spaces to hyphens — so a contents entry links to its section
    without the renderer inventing ids the headings do not carry."""
    kept = "".join(ch for ch in heading.casefold() if ch.isalnum() or ch in " -")
    return "-".join(part for part in kept.split(" ") if part)


def _contents(ir: ArtifactIR) -> list[str]:
    """A linked contents list for a chaptered document, and nothing for a memo."""
    if not chaptered(ir):
        return []
    return ["## Contents", "\n".join(
        f"- [{section.heading}](#{_anchor(section.heading)})"
        for section in ir.sections if not section.hidden
    )]
    ...
    *artifact_type* and *size_class* feed the artifact compiler purely to learn
    ...
    for section in ir.sections:
        if section.hidden and presentation.appendix != "append":
            ...
            continue
        if section.hidden and not appendix_opened and chaptered(ir):
            # One heading over the hidden run, as the page formats give it.
