...
def _asked_for_rows(world: World) -> list[tuple[str, str]]:
    """One row saying what got built against what was asked for, or none.

    A single line rather than the findings themselves: a finding runs to three
    sentences and the table is a count. The recipe on `world.json` carries the
    findings verbatim for whoever needs the words.
    """
    from .recipe import INSPIRED_BY_KEY, UNMET_KEY

    recipe = world.recipe
    asked_for = recipe.get(INSPIRED_BY_KEY)
    unmet = recipe.get(UNMET_KEY) or ()
    if asked_for is None and not unmet:
        return []
    parts = [str(recipe.get("archetype", "—"))]
    if asked_for is not None:
        parts.append(f"asked for {asked_for!r}")
    parts.append(f"unmet: {len(unmet)}")
    return [("Built as", "; ".join(parts))]
    ...
    def __init__(self, world: World) -> None:
        ...
        self.rows: list[tuple[str, str]] = [
            ...
            *([("Largest unit", _largest_unit(world))] if any(
            ...
            # Only when the recipe says the build asked for something, or could
            # not meet something. `inspect` is where a reader looks at a corpus
            # they were handed, and a supermarket built for a hospital request
            # used to look exactly like a supermarket here.
            *_asked_for_rows(world),
        ]
