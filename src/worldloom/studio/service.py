...
class Studio:
    ...
    def describe(self, project: str, revision: str | None = None) -> dict[str, Any]:
        ...
        return {**current, "resolution": {"engine": resolution.engine, "unmet": list(resolution.unmet)},
                ...
                "jobs": self.store.jobs(project), "interviews": self.interviews(project)}
    ...
    def snapshot(self, spec: ProjectSpec) -> tuple[World, Path]:
        ...
        if staging.exists():
            import shutil
            if _read(staging / "intent.json") != intent:
    ...
    def evidence(self, project: str, revision: str, row_id: str) -> dict[str, Any]:
        ...
        directory = self.path("datasets", digest([project, revision]))
    ...
    def execute(self, job_id: str, *, harness_command: str | None = None, timeout: float = 600) -> dict[str, Any]:
        ...
        if options.operation == "narrate":
            ...
            if staging.exists():
                import shutil
                if _read(staging / "job.json") != {"job": job_id, "snapshot": location.name}:
