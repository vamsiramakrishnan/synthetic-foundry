...
async function refresh() {
  ...
  if(state.company.revision!==previous){state.evals=[];state.offset=0;state.history=[];state.agent={job:null,rows:[],offset:0,total:0,verdict:""};resetCreation();}
}
async function selectCompany(id) { resetCreation(); state.nativeProposal=null; state.company=await api(`/api/projects/${id}`);state.evals=[];state.offset=0;state.history=[];state.filter="";state.agent={job:null,rows:[],offset:0,total:0,verdict:""};render(); }
...
function overview() {
  ...
  return head(esc(companyName(p)),[s.company.engine?label(s.company.engine):"",label(s.company.geo),"One persistent enterprise"].filter(Boolean).join(" · "),button("Continue interview","go-interview")+button("Open foundry","go-foundry","primary"))+journey()+workflowPanel()+
...
function agentRuns() { return state.company.jobs.filter(j=>j.options.operation==="evalrun"&&j.revision===state.company.revision); }
function agentTable() {
  const a=state.agent,rows=a.rows||[];
  const verdictBadge=(row)=>row.status==="error"?badge("Error","red"):row.passed?badge("Passed","green"):badge("Failed","amber");
  return `<div class="spaced"><div class="field"><label for="agent-verdict">Show</label><select id="agent-verdict">${[["","Every case"],["passed","Passed"],["failed","Failed"],["error","Errors"]].map(([v,t])=>option(v,t,a.verdict)).join("")}</select></div><span class="small muted">${rows.length?`${a.offset+1}–${a.offset+rows.length} of ${fmt(a.total)}`:"No graded cases match"}</span></div>
    ${rows.length?`<div class="table-wrap"><table><thead><tr><th>Case</th><th>Shape</th><th>Split</th><th>Plan</th><th>Trajectory</th><th>Outcomes</th><th>Verdict</th></tr></thead><tbody>${rows.map(row=>`<tr><td><strong>${esc(row.use_case)}</strong><br><span class="muted tiny">${esc(row.query.slice(0,120))}</span><br><code class="tiny">${esc(row.case_id.slice(0,12))}</code></td><td>${esc(row.shape)}<br><span class="muted tiny">${esc(label(row.failure))}</span></td><td>${esc(row.split)}</td><td>${row.plan?`${axis(row.plan.score)}${row.plan.missing_nodes.length?`<br><span class="tiny muted">missing ${esc(row.plan.missing_nodes.join(", "))}</span>`:""}`:"unobserved"}</td><td>${row.trajectory?`${axis(row.trajectory.score)}<br><span class="tiny muted">failures ${esc(row.trajectory.failures)}${row.trajectory.safety.length?` · ${esc(row.trajectory.safety.join(", "))}`:""}</span>`:"unobserved"}</td><td>${row.outcomes?`${axis(row.outcomes.score)}<br><span class="tiny muted">${esc(row.outcomes.structured)} met${row.outcomes.collateral?` · ${row.outcomes.collateral} collateral`:""}</span>`:"unobserved"}</td><td>${verdictBadge(row)}${row.error?`<br><span class="tiny muted">${esc(row.error.slice(0,160))}</span>`:""}</td></tr>`).join("")}</tbody></table></div>`:""}
    <div class="actions end">${button("Previous","agent-prev","",a.offset===0?"disabled":"")}${button("Next","agent-next","",a.total<=a.offset+rows.length?"disabled":"")}</div>`;
}
function agentPanel() {
  const jobs=agentRuns(),ready=Boolean(report());
  const actions=button("Grade reference agent","evalrun","primary",ready?"":"disabled")+button("Evaluate connected harness","evalrun-harness","",ready&&state.harness?"":"disabled")+button("Grade plans only","evalrun-plan","quiet",ready?"":"disabled");
  return `<section class="panel"><div class="panel-head"><div><h2>Agent grades</h2><p class="muted small">Plan, trajectory and outcomes graded separately on this revision's connector cases. The reference agent is the executable ceiling; a connected harness is compared to it.</p></div><div class="actions">${actions}</div></div>${jobs.length?jobs.map(j=>{
    const s=j.result?.summary,p=j.progress,running=["queued","running"].includes(j.status);
    return `<article class="unit"><div class="spaced"><strong>${esc(agentLabel(j))}</strong>${badge(label(j.status),statusColor(j.status))}</div>${s?`<p>${fmt(s.passed)} / ${fmt(s.graded)} passed · ${fmt(s.errors)} error rows excluded · plan ${axis(s.means.plan)} · trajectory ${axis(s.means.trajectory)} · outcomes ${axis(s.means.outcomes)}</p><div class="actions">${(s.by_shape||[]).map(part=>badge(`${part.key}: ${part.passed}/${part.graded}`)).join("")}</div>`:running&&p?`<p>${fmt(p.graded)} / ${fmt(p.total)} graded so far</p>`:""}${j.error?`<p class="small">${esc(j.error)}</p>`:""}<div class="actions">${j.status==="complete"?button(state.agent.job===j.id?"Refresh graded cases":"Show graded cases","load-agent-results","quiet",`data-id="${esc(j.id)}"`):""}${["failed","interrupted"].includes(j.status)?button("Resume","retry","quiet",`data-id="${esc(j.id)}"`):""}</div>${state.agent.job===j.id?agentTable():""}</article>`;
  }).join(""):`<p class="small muted">${ready?"No agent graded yet. Grade the reference agent first; its pass rate is the ceiling of this dataset, not a claim about any model.":"Generate the connector queryset first; it is the case set an agent is graded on."}</p>`}</section>`;
}
function evalsPage() {
  const r=report();
  return head("Evaluations","Inspect the exact queries, ownership and evidence admitted by the compiler, then grade agents on them.",button("Refresh queries","load-evals")+button("Generate evalset","compile","primary",!connectorReady()?"disabled":""))+agentPanel()+
...
function nativeCalibrationResults(result) {
...
async function run(operation, message="", extra={}) { const job=await api(route("run"), {revision:state.company.revision, options:{operation,message,...extra}});await refresh();render();notify(`${label(operation)} queued. You can keep exploring while it runs.`);return job; }
...
worldloom studio serve --harness codex --allow-native-writes</pre><p class="small muted">Custom adapters can use <code>--harness-command</code>. Commands are configured when Studio starts.</p><p>Without an adapter, use <strong>Export request</strong> and <strong>Import response</strong> in the interview.</p>`);return;}
  ...
  if(name==="evalrun"){await run("evalrun");state.page="evals";render();return;}
  if(name==="evalrun-harness"){await run("evalrun","",{evalrun_agent:"harness"});state.page="evals";render();return;}
  if(name==="evalrun-plan"){await run("evalrun","",{evalrun_mode:"plan"});state.page="evals";render();return;}
  if(name==="load-agent-results"){state.agent={...state.agent,offset:state.agent.job===target.dataset.id?state.agent.offset:0};await loadAgentResults(target.dataset.id);return;}
  if(name==="agent-next"||name==="agent-prev"){state.agent.offset=Math.max(0,state.agent.offset+(name==="agent-next"?25:-25));await loadAgentResults(state.agent.job);return;}
