...
`evaluate` indexes the corpus with a deliberately mediocre retriever — naive
...
— the stronger claim, because a family low under **both** is hard for a
...
`worldloom stats` is `evaluate`'s sibling for a different question — not "is
this hard to retrieve from" but "what does it actually contain": document
