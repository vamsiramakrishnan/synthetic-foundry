...
| Field | What it is |
| --- | --- |
| `id` | `<artifact_id>/<section>`. Copy it into your response unchanged; it is the join key. |
...
The whole batch is reviewed together, not stopped at the first failure, so you get
...
the reference: `` finished {{fact:FACT-0019}} late ``. The rule exists for one
reason: a typed-out figure is a copy, and the renderer cannot keep a copy in
sync with the ledger the way it keeps a reference in sync. Fix: find the digit,
find the fact ID in your request whose `statement` produced that value, replace
the digit with `{{fact:THAT-ID}}`. If no fact in your request carries that
value, you should not be stating it at all; see `unsupported_claim`.

**`unsupported_claim`**: a claim in your response cites a fact ID not in this
request's `allowed_fact_ids`. That means not in *this* request, which is
narrower than not in the corpus. The same fact can be perfectly real and cited
correctly by a different section and still be wrong here, because the request
is what bounds what this particular author, writing this particular document,
...
truth.** The check is `valid_from <= knows_as_of` (had the fact come into
existence yet), never `holds_at(knows_as_of)` (was the fact *current* at that
moment). The two questions are kept apart for a reason. If it were
...
never as a flat present-tense assertion. This is how a real incident RCA
...
the hypothesis is still live: "First read was `{{fact:FACT-0004}}`... It did
not hold") versus `ART-0007/Root
...
same incident. `written_by` and `voice` are handed to you so you don't
...
2. Fix that and only that. Do not touch sections that were not named.
...
what its own docstring says it is: "not a stand-in for a language
...
in-process API caller to fall back to. One existed (`narrate auto`, calling
hosted model APIs directly) and was deleted, because the writer this project
