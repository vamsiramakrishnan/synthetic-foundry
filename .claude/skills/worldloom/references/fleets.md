...
compared two builds of a bank running more than one period**, which is where a
carry-forward defect would live.
...
combinations it missed. The ordering matters: one unvaried axis is a
...
built it. Separating "chose badly" from "cannot reach" is the reading that
matters.
...
The rule to keep: **every hole should have a declaration behind it.** A
hole with no declaration is an assumption in the planner, and that is how the
multi-period bank gap hid for as long as it did.
