...
`register(name, render_all_fn)`. A renderer's contract is narrow on purpose:
it reads an `ArtifactIR` and nothing else — no facts, no world, no model. That
...
— a save today and a save tomorrow of an *identical* document differ in bytes.
...
stream you were handed — pulling from the shared stream is exactly the
...
only because of this exact problem.
...
strictly after everything the frozen narration depends on — and check your
...
Worth knowing before you extend near them, stated honestly rather than
glossed over:
