...
over the whole set of fingerprints at once, never per artifact, for exactly
this reason. If you find yourself asking "is this artifact diverse", that is
...
style genome, density bucket, and section count. Deliberately not its prose or
its facts: two artifacts about completely different business units collapse to
the same fingerprint when they read the same way, which is exactly the
...
explain, the exact "passes locally, fails in CI" defect determinism exists to
...
frequency. That width is deliberate, not arbitrary: a unigram measure (counting
...
precisely the thing "the same outline every time" repeats. A batch of
...
same contract as `grammar.check`. Defaults, and why each one exists:
...
- **`max_repetition_run`** (default 2) — the longest run of *consecutive*
  identical shapes, in the order the batch was given. Order-sensitive on
  purpose: three identical memos back to back is a worse reading experience
