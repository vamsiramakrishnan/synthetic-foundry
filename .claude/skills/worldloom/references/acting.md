...
The difference from narration matters. Narrating, you are the
...
| `facts` | **Everything you know.** Each carries `statement`, `authority`, `valid_from`, and three fields with no equivalent in narration: `learned_at`, `learned_via`, and `confidence`. |
...
repository. That boundary is what the episode is for: an evaluation asking "who
knew the root cause before the close decision" only has an answer because it was
kept.
...
The second is the one to show a user. Two people woken by the same failure
