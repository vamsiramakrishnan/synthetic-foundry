...
Lowering it makes every question type harder, which is a way to check that a
family isn't passing only because it's allowed to return half the corpus.
`--retriever` picks which ranking family scores the corpus: `bm25` (the
default, unchanged), `tfidf`, `embedding`, `both` (the two lexical baselines)
...
`src/worldloom/evaluate/`, because a judge model would put the thing under
...
into a `Passage` that carries its fact IDs, authority, and timestamp. Nothing
about "did this read well" is scored here; that's the narration stage's job,
enforced by `claims.py` instead.
...
time, authority, or provenance. It ranks purely on keyword overlap across
every document in the corpus at once, as if they'd all been written
simultaneously and were all equally to be trusted. It exists to be beaten.
More precisely, it exists to **fail** in a specific, informative way:
...
`src/worldloom/evaluate/tfidf.py` is the second one: TF-IDF cosine, a
different ranking family rather than a copy of the first with different
...
is vector-space (term frequency is log-*damped*, a different nonlinearity from
saturation; length is normalised by each document's own Euclidean vector norm,
...
they differ is *which* keyword patterns each one happens to reward, and that
is the axis the credibility reading below depends on. Tokenisation is
...
- **disagreement**: the two retrievers split on enough cases in that family
  ...
  it as a finding about the corpus that belongs in a corpus card, not as a
  number to average into the other two categories.

`compare()` computes this per case, not from matching aggregate pass rates.
Two retrievers passing the same 3-of-6 for a family while failing three
entirely different cases would look identical on an aggregate-only comparison,
and that is the kind of disagreement this exists to catch.
...
- **`lexical trap`**: the keyword baselines fail and the semantic retriever
  solves it. **The family was never hard**, whatever the BM25 column
...
semantic. That is still failing, so still hard, but the gap says part of what
BM25 was failing on was vocabulary rather than authority. And a dense retriever
can pass a case **for the wrong reason**: on the five-world mosaic the
static-embedding pin passes `temporal_state` five times out of fifteen, always
the same question, by preferring a short triage page whose text is dominated by
the word the question uses, with no notion of *when* anything was written. A
pass is not evidence of a capability; read `-v` and check what was returned.
...
From `src/worldloom/generators/evaluation.py`, where each `_Taxonomy` method
builds one or more `EvaluationType`s. For each, ask what capability a retriever needs
...
- **`numerical_comparison`** (`numerical_comparison()`): several facts that have
  ...
  from the corpus, but not from any single page: it needs the passages that carry
...
`causal_multi_hop` is the corpus working as designed.** That is the result,
not a caveat on it.

One caveat only became sayable once a non-lexical retriever ran: a *low*
keyword score is not by itself difficulty. It is difficulty only if the
...
The intuition reads backwards the first time, so state it plainly: **a rising
baseline score is bad news.** Unless someone improved the retriever itself (a
different index, added time-awareness, added authority weighting), a rising
score on the hard types means the corpus got easier, not that anything got
better. This is why CI asserts the hard types stay hard (see
`tests/test_evaluate.py`: `test_the_baseline_fails_on_knowing_when`,
...
known") and it was wrong: filtering by the cutoff **is** the capability under
test. Handing the baseline a pre-filtered pool measures a temporal-aware
retriever that doesn't exist in this repository, and the score came back
perfect, which felt like success and meant the opposite: the test had stopped
testing anything. `score.py` now grades the top hit **unfiltered**
...
The corpus isn't unanswerable; the baseline is just the wrong tool for it.
...
generator never models at all: people costs, suppliers,
...
both are named constants in `stats.py`, not magic numbers), fact-reference density
...
reference, so a comparison against one would be the kind of unverifiable
claim this whole effort exists to stop making. Report, don't grade. The one legitimate comparison this module makes is `worldloom stats
...
Outliers are scored on **median absolute deviation**, not a z-score, for a
reason worth keeping: an outlier inflates the standard deviation it would
be measured against, so several of them mask each other and the detector fails
hardest when there is most to find. The decomposition refits once with
