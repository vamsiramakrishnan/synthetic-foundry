...
| `organisation` | headcount, span, levels, functions — plus `divisions`, how many lines of business | `roles.from_shape` → `roles.review`; divisions via `worldloom.divisions` — the knob corpus size actually follows, because the close fans out per division, not per employee |
