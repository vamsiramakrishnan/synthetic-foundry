...
run 18,484 to 2,712,716 per head: that is the omnichannel_retailer and the
...
The envelope is **computed from the archetype registry**, not typed in: six
...
`generators/finance` never consults headcount). Register a seventh archetype
and the envelope moves on its own.

The other refusals are composed rather than restated:

- `facets.resolve` on an empty intersection: *`listing:mutual` wants a 16-26%
  margin and `margin_profile:premium` wants 48-62%, and no company is both.*
...
- `organisation.headcount > employees`: a corpus cannot name more people than
  the company employs, because every person the corpus names *is* an employee
  and the headcount fact would contradict the org chart in the same workbook.
...
- **A `geo` with no identity to carry it.** A locale has two halves. The figure
  ...
  `name_pools`/`regions`/`headquarters`. A description that names Germany
  and no company gets German digits and Australian staff, and is told so.
- **A named rival**, one `unmet` line per entry in `rivals`. Nothing here mints
  an entity for a company that is not this one, and lore asserting one would
  constrain no consulted target. Say what the market *does* to pricing with
  `facets: {"competition": …}` instead: that facet resolves into the
  margin-erosion range the physics engine actually reads, where a named
  rival resolves into nothing.
