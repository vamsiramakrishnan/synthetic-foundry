...
Both are cohort-only. The lint refuses either without a `cohort`:
...
The invariant to declare is **`rolls-up-to`**, on the cohort kind naming its
parent, not `sums-to`, which decomposes one period across *subjects*; this
decomposes one subject across *cohort periods*, and a check looking on the
wrong axis passes vacuously.
