...
- **The doctypes lint is the boundary for paperwork.** An authored type that
  cites kinds nothing produces still *compiles*, into a document that is
  carried, cited, and says nothing, so `worldloom.doctypes.lint` findings
  are read and fixed, not shipped.
...
- **Source-blindness at narration.** The brief is the boundary at every
  ...
  `src/` narrated 115 sections, and that is the contract, not a stunt.
