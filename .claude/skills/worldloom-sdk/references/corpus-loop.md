...
`.blueprint` on the mutant is the ancestor's, deliberately: the recipe records
what was made and is what was patched; the blueprint tells a fan-out loop
...
- **Generations**: `from worldloom import evolve` — `evolve.evolve(...)` is a
  ...
  it builds through the `worldloom` executable deliberately, so its members
