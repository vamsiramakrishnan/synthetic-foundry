...
# The refine loop that is not here
...
a fair question of any corpus whoever narrated it, not least as the check that
...
and `.mcp.json` wires them into any MCP client, so a session can ask those
