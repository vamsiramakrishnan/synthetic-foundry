...
It is a **composer, not an engine**. Every field resolves into a seam the
flags already depend on (`archetypes.get`, `vocabulary.spoken`, `facets.resolve`,
...
* **It reports what it cannot honour, rather than dropping it.** A trading year
  ...
  the same reason.
* **It is never recorded.** A pack is embedded in the recipe as written,
  because the pack *is* how the world was made. A specification resolves to
  consequences and the recipe records *those*, as `--facet` records
...
of businesses and naming no company at all. Supplying `identity.company_name` is
...
The same principle decides the other three verticals, and you should know
...
one, `worldloom pack targets <engine>` lists which lore the engine consults,
